...
| Asset formats and resource binding | Bounded compact streams plus admitted caller-owned texture tables or pre-acquired fixed-slot residency sets resolve model identifiers into checked PVR surfaces and materials; the host inspector applies the target validator to raw little-endian streams | Runtime binding and build-time admission are complete without a global namespace or second container format; source conversion and content authoring remain separate tools. |

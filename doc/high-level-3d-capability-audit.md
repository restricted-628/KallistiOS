...
| Asset formats and resource binding | Bounded compact streams plus admitted caller-owned texture tables or pre-acquired fixed-slot residency sets resolve model identifiers into checked PVR surfaces and materials; host tools convert an explicit triangulated OBJ subset, resolve repeated material names into persistent texture-state transitions, optionally join compatible order-adjacent triangles into bounded strips, and apply the target validator to raw little-endian streams | Core conversion, order-preserving strip joining, multi-material texture partitioning, runtime binding, and build-time admission are complete without a global namespace or second container format; global topology reordering, library material-property conversion, and content authoring remain separate policy. |

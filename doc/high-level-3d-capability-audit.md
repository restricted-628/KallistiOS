...
| Asset formats and resource binding | Bounded compact streams plus admitted caller-owned texture tables or pre-acquired fixed-slot residency sets resolve model identifiers into checked PVR surfaces and materials; host tools convert an explicit triangulated OBJ subset and apply the target validator to raw little-endian streams | Core conversion, runtime binding, and build-time admission are complete without a global namespace or second container format; multi-material partitioning, offline strip optimization, and content authoring remain separate policy. |

...
| Object hierarchy | Parent-before-child compact-model traversal composes static matrices or sampled caller-owned TRS poses with bounded workspace and callbacks; explicit node policy supports component suppression, hidden nodes, and noncontiguous descendant pruning; checked PCH1 version 1/2 sections bind stable model ordinals into that same evaluator | Keep scene policy and model lifetime outside the traversal core. |

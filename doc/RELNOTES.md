...
* Added caller-owned compact-model draw caches for ordinary strips. One-time
  admission retains decoded strip state, assembled 32-byte PVR vertices,
  canonical deformation inputs, and original vertex indices in an exact
  queried footprint. Repeated emission reads neither compact stream and can
  bind caller-owned skin or morph results plus per-frame lighting without
  creating a scene owner, worker, allocator, or hidden resource namespace.

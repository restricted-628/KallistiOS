...
static int cache_emit_filtered(
    ...
    for(strip_index = 0; strip_index < cache->strip_count; ++strip_index) {
        ...
        if((admitted ? pvr_geometry_project_packed_inplace(
                            workspace, strip->vertex_count, sizeof(*workspace),
                            object_to_screen) :
...
int pvr_chunk_model_two_volume_cache_draw_prepare(
    ...
    if(!draw || ((uintptr_t)draw &
                 (_Alignof(pvr_chunk_two_volume_cache_draw_t) - 1u))) {
        errno = EINVAL;
        return -1;
    }
    if(two_volume_cache_valid(cache) < 0 ||
       address_range(cache, sizeof(*cache), &cache_start, &cache_end) < 0 ||
       address_range(cache->storage, cache->storage_bytes,
                      &storage_start, &storage_end) < 0 ||
       address_range(draw, sizeof(*draw), &draw_start, &draw_end) < 0)
...
static int two_volume_emit_preflight(
    ...
    if((!admitted && two_volume_cache_valid(cache) < 0) ||
...
static int two_volume_cache_emit_filtered(
    const pvr_chunk_two_volume_cache_t *cache,
    ...
    for(strip_index = 0; strip_index < cache->strip_count; ++strip_index) {
        ...
        for(index = 0; index < strip->vertex_count; ++index) {
            ...
            pvr_deform_vertex_t resolved;
            pvr_chunk_two_volume_vertex_t vertex;
            uint8_t *destination = (uint8_t *)workspace + index * cache->vertex_size;
            uint32_t command = index + 1u == strip->vertex_count ?
                               PVR_CMD_VERTEX_EOL : PVR_CMD_VERTEX;
            ...
            if(admitted && !prepare_vertex) {
                /* No callback needs a maximum-sized union: retain the native
                   packet footprint and update only position/command fields. */
                memcpy(destination, (const uint8_t *)cache->vertices +
                       cached_index * cache->vertex_size, cache->vertex_size);
                memcpy(destination, &command, sizeof(command));
                memcpy(destination + 4u, &deformation->position.x, sizeof(float));
                memcpy(destination + 8u, &deformation->position.y, sizeof(float));
                memcpy(destination + 12u, &deformation->position.z, sizeof(float));
                continue;
            }
            ...
            memcpy(&vertex, &command, sizeof(command));
            memcpy(destination, &vertex, cache->vertex_size);
        }
        ...
        if((admitted ? pvr_geometry_project_packed_inplace(
                            workspace, strip->vertex_count, cache->vertex_size,
                            object_to_screen) :
                       pvr_geometry_project_vertices(workspace, workspace_count,
                            &stream, object_to_screen, NULL)) < 0)
            goto fail;
        if(begin_strip) {
            errno = 0;
            if(begin_strip(strip, data) < 0) {
...
int pvr_chunk_model_two_volume_cache_emit_filtered(
    ...
    pvr_chunk_cache_prepare_two_volume_vertex_t prepare_vertex,
    void *data, pvr_chunk_cache_result_t *result) {
    return two_volume_cache_emit_filtered(cache, object_to_screen, sink,
        workspace, workspace_count, filter_strip, begin_strip, resolve_vertex,
        prepare_vertex, data, result, 0);
}
...
int pvr_chunk_model_two_volume_cache_draw_emit(
    ...
    return two_volume_cache_emit_filtered(&draw->cache, object_to_screen, sink,
        workspace, workspace_count, filter_strip, begin_strip, resolve_vertex,
        prepare_vertex, data, result, 1);
}

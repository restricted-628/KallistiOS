...
level handling, codebook type checking, and a blocking DMA upload to a temporary
linear surface. The program aborts on a failed invariant and prints
`RESULT: PASS (PVR texture surface)` after 120 rendered frames.

Emulation validates software layout, normal transfer flow, and visible
rectangle placement. Physical DMA timing and updates concurrent with texture
sampling remain hardware validation items.

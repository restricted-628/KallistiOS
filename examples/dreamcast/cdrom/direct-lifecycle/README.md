...
1. reads and decodes `REQ_MODE`;
2. sends `SET_MODE` with the exact settings just read, exercising the PIO
   host-to-drive phase without changing the effective configuration;
3. verifies the mode page;
4. runs the bounded soft-reset/readiness sequence;
...
the drive identification, pre/post-reset mode values, and probe result.
Physical-drive timing remains a separate hardware validation gate.

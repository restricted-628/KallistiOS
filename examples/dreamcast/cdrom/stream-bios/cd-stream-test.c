/* KallistiOS ##version##
   ...
   Explicit legacy BIOS PIO/DMA streaming demonstration.
   The default direct-session example lives in ../stream.
*/
...
static int cd_stream_test(uint32_t lba, uint8_t *buffer, size_t size, bool dma) {
    ...
    rs = cdrom_bios_stream_start(lba, size / 2048, dma);
    ...
    cdrom_bios_stream_set_callback(cd_stream_callback, (void *)&cb_count);
    rs = cdrom_bios_stream_request(buffer, size / 2, 1);
    ...
    rs = cdrom_bios_stream_progress(&cur_size);
    ...
    rs = cdrom_bios_stream_request(buffer + (size / 2), size / 2, 1);
    ...
    rs = cdrom_bios_stream_stop(false);
